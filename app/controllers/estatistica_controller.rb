class EstatisticaController < ApplicationController
  def index
    ...
    resultado = []
    (primeiro_mes..ultimo_mes).each do |mes|
      ...
      resultado << {
        ...
        pago: {
          ...
          pix: query.filter { |key| key[0] == mes }.filter { |key| key[1] == 'pago' }.filter { |key| key[4] == 'pix' }.values.sum,
          cartao: query.filter { |key| key[0] == mes }.filter { |key| key[1] == 'pago' }.filter { |key| key[4] == 'cartao' }.values.sum,
          dinheiro: query.filter { |key| key[0] == mes }.filter { |key| key[1] == 'pago' }.filter { |key| key[4] == 'dinheiro' }.values.sum
        }
      }
    end
    ...
    total_alunos = users_sede.sum(&:alunos)
    # render json: total_alunos
    render json: {total_alunos: total_alunos, sede: users_sede, receita_maxima: receita_maxima, receita_tota_pago: receita_pago, receita_total_atrasada: receita_atrasada, mensal: resultado}, adapter: nil
  end
end
